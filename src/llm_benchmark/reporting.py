...
def aggregate_runs(runs_root: Path) -> dict[str, Any]:
    ...
    for run_dir in run_dirs:
        ...
        if provenance:
            ...
            if provenance.get("generations_from"):
                reused_from.append(provenance["generations_from"])
        ...
        for model in summary["models"]:
            ...
            for key in (
                ...
                "telemetry_source",
                # Repetition bookkeeping (reliability / speed suites). Present
                # only when the experiment ran repetitions > 1.
                "repetitions",
                "consistency_rate",
                "unstable_task_ids",
                "truncated_rows",
    ...
    for suite_name, model_buckets in sorted(suite_model_totals.items()):
        ...
        for model in model_buckets.values():
            ...
            ci_low, ci_high = wilson_interval(model["passes"], model["total"])
            models.append(
                {
                    **model,
                    "pass_rate": round(model["passes"] / total, 4),
                    # 95 % Wilson interval on the pass rate. These suites are
                    # small, so the interval is usually wide — that is the
                    # point: it stops a 1-task difference reading as a result.
                    "ci_low": round(ci_low, 4),
                    "ci_high": round(ci_high, 4),
                    "ci_margin": round((ci_high - ci_low) / 2, 4),
    ...
    return {
        ...
        "conditions": {
            ...
            # Present when the answers were generated by an earlier run and
            # only re-scored here. Surfaced so the page cannot read as a fresh
...
def _conditions_markdown(conditions: dict[str, Any]) -> list[str]:
    ...
    reused = conditions.get("generations_from")
    if reused:
        origin = reused.get("git_commit") or "an unrecorded commit"
        lines.append(
            f"**Answers were not generated by this run.** They come from `{reused.get('bundle')}` "
            f"(generated at `{origin[:12] if reused.get('git_commit') else origin}`) and were re-scored "
            "here. Scoring is offline and deterministic, so this measures the scorer's verdict on the "
            "same answers — not a fresh sample of the models."
        )
        lines.append("")
...
def render_markdown_report(aggregate: dict[str, Any]) -> str:
    ...
    for suite in aggregate["suites"]:
        ...
        for model in suite["models"]:
            ...
            consistency_cell = "-" if consistency is None else f"{consistency:.0%}"
            if performance_probe:
                # Every row of a timing probe "passes" by construction; printing
                # 100 % next to real pass rates would invent a quality result.
                rate_cell, ci_cell = "n/a", "n/a"
            else:
...
def render_html_report(aggregate: dict[str, Any]) -> str:
    ...
    Implementation lives in :mod:`llm_benchmark.reporting_html` to keep
...
# Suites whose pass column is synthetic: every row passes because the suite
# exists to capture timing, not to score answers.
_PERFORMANCE_PROBE_SUITE_KEYS = ("openclaw_speed", "sustained_throughput")


def _is_performance_probe(suite: dict[str, Any]) -> bool:
    """True when a suite's pass rate must not be read as a quality result.

    Prefers the explicit ``scoring`` flag written by the suite runner and falls
    back to the suite name so runs recorded before 0.5.3 are handled too.
    """
    if str(suite.get("scoring") or "") == "performance_probe":
        return True
    name = str(suite.get("suite") or "")
    return any(name == key or name.startswith(f"{key}_") for key in _PERFORMANCE_PROBE_SUITE_KEYS)
    ...
    *,
    ...
    for suite_name in selected_suites:
        ...
        for model in suite.get("models", []):
            ...
            if performance_probe:
                score_part = f"timing probe ({model['total']} generations)"
            else:
                low, high = wilson_interval(model["passes"], model["total"])
                score_part = (
                    f"pass rate {float(model['pass_rate']) * 100:.2f}% "
                    f"({model['passes']}/{model['total']}, 95% CI {low:.0%}–{high:.0%})"
                )
            consistency = model.get("consistency_rate")
            if consistency is not None and (model.get("repetitions") or 1) > 1:
                score_part += f", consistency {consistency:.0%}"
            lines.append(f"- {model['model']}: {score_part}, avg TTFT {ttft}, avg speed {tok_s}")
