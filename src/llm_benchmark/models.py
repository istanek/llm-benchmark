...
class HarnessProvenance(BaseModel):
    ...
    # Set when the answers were not generated by this run: scoring is offline
    # and deterministic, so a scorer fix can be applied to stored generations
    # instead of spending hours re-asking the models. Records where they came
    # from and which commit produced them, because "measured at commit Y" and
    # "generated at X, scored at Y" are different claims and the second one
    # should never be able to pass as the first.
    generations_from: dict[str, Any] | None = None
