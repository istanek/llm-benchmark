...
def parse_benchmark_request(request: str, available_models: list[str]) -> BenchmarkPlan:
    ...
    if any(token in normalized for token in ("mutated", "contamination")):
        selected_suites.append("code_generation_mbpp_mutated")
    elif any(token in normalized for token in ("mbpp",)):
...
def run_openclaw_speed_suite(
    *,
    ...
    """Timing probe. Rows always "pass" — the payload is the captured metrics.

    The summary is tagged ``scoring: "performance_probe"`` so reporting does not
    present the resulting synthetic 100 % next to real quality pass rates.
    """
    ...
    for model_config in model_configs:
        ...
        # Discarded generations that absorb first-call costs (weight load, cache
        # warm-up) so they do not land in the reported TTFT / tok-s averages.
        for warmup_index in range(warmup_runs):
            if progress_callback:
                progress_callback(
                    f"  {model_config.name} → warmup {warmup_index + 1}/{warmup_runs} (discarded)"
                )
            warmup_task = suite.tasks[0]
            warmup_prompt = (
                warmup_task.prompt
                if not warmup_task.context
                else f"{warmup_task.context}\n\n{warmup_task.prompt}"
            )
            backend.generate(warmup_prompt, suite_sampling)
        for idx, task in enumerate(suite.tasks, start=1):
            for repetition in range(1, repetitions + 1):
                if progress_callback:
                    suffix = f" rep {repetition}/{repetitions}" if repetitions > 1 else ""
                    progress_callback(
                        f"  {model_config.name} → speed task {idx}/{total_tasks} ({task.task_id}){suffix}"
                    )
    ...
    summary = build_summary(run_rows, suite, backend_config, scoring="performance_probe")
...
def run_benchmark_bundle(
    *,
    ...
    repetitions = max(1, int(getattr(experiment, "repetitions", 1) or 1))
    warmup_runs = max(0, int(getattr(experiment, "warmup_runs", 0) or 0))
    ...
    for suite_index, suite_name in enumerate(plan.selected_suites, start=1):
        ...
        if suite_name == "hallucination_grounding":
        ...
        elif suite_name in {"code_generation", "code_generation_mbpp", "code_generation_mbpp_mutated"}:
            suite = (
                load_mbpp_mutated_suite(repo_root)
                if suite_name == "code_generation_mbpp_mutated"
                else load_mbpp_suite(repo_root)
            ...
            summary = run_sustained_throughput_suite(
                ...
                model_configs=model_configs,
                sampling=experiment.sampling,
                warmup_generations=warmup_runs,
                progress_callback=progress_callback,
            )
        elif suite_name in {
