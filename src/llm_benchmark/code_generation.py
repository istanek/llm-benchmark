...
configuration targets a single local machine (see docs/extensions-spec.md).
"""
...
def sandbox_run(
    ...
    *,
    ...
    """
    with tempfile.TemporaryDirectory(prefix="llm-bench-codegen-") as scratch:
...
def load_mbpp_mutated_suite(repo_root: Path) -> SuiteDefinition:
    """MBPP with the same problems in different words — a contamination probe.

    Read as a delta against ``load_mbpp_suite``: identical problems, renamed
    entry points, paraphrased prose, recomputed examples. A model that drops
    far more than the others is showing recall rather than ability. The
    absolute score means little on its own, since unfamiliar naming costs every
    model something. Generated by ``scripts/build_mbpp_mutants.py``.
    """
    return load_suite_definition(repo_root / "data" / "code" / "code_generation_mbpp_mutated_v1.json")
