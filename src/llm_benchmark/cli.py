...
from llm_benchmark.quick import (
    QUICK_TASK_ID,
    build_quick_suite,
    default_save_dir,
...
from llm_benchmark.orchestration import (
    BenchmarkPlan,
...
APP_HELP = """[bold cyan]
  ...
  ╚══════╝╚══════╝╚═╝     ╚═╝
[/bold cyan]
[bold]LLM Benchmark[/bold] // pixel CLI for local LLM testing
"""
...
def maybe_print_banner() -> None:
    if not sys.stdout.isatty():
        return
    # SPARK_BENCH_NO_BANNER is the pre-rename name, still honoured.
    if os.environ.get("LLM_BENCH_NO_BANNER") == "1" or os.environ.get("SPARK_BENCH_NO_BANNER") == "1":
        return
    ...
    art = Text(
        "\n".join(
            [
                ...
                "  ███████╗███████╗██║ ╚═╝ ██║",
                "  ╚══════╝╚══════╝╚═╝     ╚═╝",
                "   llm-benchmark // local model trials",
            ]
        ),
        style="bold #7dd3fc",
    )
    subtitle = Text("pixel CLI for local LLM benchmarking", style="#f59e0b")
    console.print(
        Panel.fit(
            Text.assemble(art, "\n", subtitle),
            border_style="#38bdf8",
            padding=(1, 2),
            title="[bold white]LLM Benchmark[/bold white]",
...
def detect_ollama_model_tags(backend_config: object) -> set[str]:
    ...
    Delegates to :func:`llm_benchmark.model_registry.detect_ollama_models`
...
WIZARD_BANNER = [
    ...
    "  ██████╔╝╚██████╔╝██╔╝ ██╗",
    "  ╚═════╝  ╚═════╝ ╚═╝  ╚═╝",
    "  llm-benchmark // benchmark wizard",
]
...
def main(
    ...
    experiment: Path | None = typer.Option(
        ...
        exists=True,
        dir_okay=False,
        help="Experiment YAML to load (default: configs/experiments/ollama-baseline.yaml).",
    ),
    platform: str | None = typer.Option(
        None,
        "--platform",
        "-p",
        help="Platform name (default: local).",
    ),
) -> None:
    if ctx.invoked_subcommand is None:
        from llm_benchmark.shell import run_shell
...
def shell_command(
    ...
    """Launch the interactive llm-benchmark shell."""
    from llm_benchmark.shell import run_shell
...
def shell_entrypoint() -> None:
    """Console-script entry point for `llm_benchmark`."""
    import sys
    ...
    if run_suite:
        ...
        if run_suite in {"hallucination_grounding", "hallucination_grounding_v1"}:
            ...
            summary = run_sustained_throughput_suite(
                ...
                model_configs=model_configs,
                sampling=experiment_spec.sampling,
                warmup_generations=experiment_spec.warmup_runs,
            )
        else:
