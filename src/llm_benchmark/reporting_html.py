...
def _render_conditions_section(conditions: dict[str, Any]) -> str:
    ...
    if reused:
        origin = reused.get("git_commit")
        parts.append(
            '<p class="note"><strong>Answers were not generated by this run.</strong> They come from '
            f"<code>{_esc(reused.get('bundle'))}</code>"
            + (f" (generated at <code>{_esc(origin[:12])}</code>)" if origin else "")
            + ", and were re-scored here. Scoring is offline and deterministic, so this is the "
            "scorer's verdict on the same answers — not a fresh sample of the models.</p>"
        )
...
def render_canonical_report_html(
    ...
    *,
    ...
    if request:
        ...
        body.append(
            '<p class="tagline">Reproducible benchmark of locally-served '
            "models — reliability, latency, throughput, and code "
    ...
    body.append("<footer>")
    body.append(
        "Generated by <code>llm-benchmark</code> · "
...
def _render_suite_block_html(suite: dict[str, Any]) -> str:
    ...
    parts.append(_render_suite_table(models, performance_probe=_is_performance_probe(suite)))
    parts.append("</section>")
    return "".join(parts)


def _render_suite_table(
    models: list[dict[str, Any]], *, performance_probe: bool = False
) -> str:
    """Shared per-model results table with color-coded pass-rate cells.

    The pass rate travels with its 95 % Wilson interval so a small-n suite
    cannot be read as if a 1-task gap separated two models. Timing probes show
    no pass column at all — theirs is synthetic.
    """
    ...
    for model in models:
        ...
        consistency_cell = "—" if consistency is None else f"{float(consistency) * 100:.0f}%"
        if performance_probe:
            rate_html = "<td class='num'>n/a</td>"
            ci_cell = "n/a"
        else:
            rate_html = _cell_pct_html(model.get("pass_rate"))
            low, high = model.get("ci_low"), model.get("ci_high")
            ci_cell = (
                "—"
                if low is None or high is None
                else f"{float(low) * 100:.0f}–{float(high) * 100:.0f}%"
            )
        parts.append(
            "<tr>"
            ...
            f"{rate_html}"
            f"<td class='num'>{_esc(ci_cell)}</td>"
            f"<td class='num'>{consistency_cell}</td>"
...
def _render_suite_openclaw_speed(models: list[dict[str, Any]]) -> str:
    """Speed probe: TTFT (lower=better, inverted) · tok/s · generations.

    No pass-rate panel: every row of a timing probe passes by construction, so
    charting it would draw three identical 100 % bars and read as a quality
    result. The sample count is shown instead.
    """
    count_rows = [
        (str(m.get("model", "?")), float(m.get("total") or 0.0)) for m in models
    ]
    ttft_rows = [
    ...
    return (
        ...
        + "</div>"
        '<div class="panel"><h4>Timed generations</h4>'
        + _svg_bars(count_rows, value_format="num")
        + "</div>"
        "</div>"
    )
